...
/*
$query="select * from pvuri where pv_nou='DA'";
$caut=mysqli_query($con,$query);$rezultat=mysqli_fetch_array($caut,MYSQLI_ASSOC);
if ($rezultat['pv_nou']=="DA"){  $g1="";  $g2="";  $g3="";  $g4=""; $l1="";  $l2="";$l3="";$l4="";
  ...
  $cont=0;  while ($rezultat1=mysqli_fetch_array($caut1,MYSQLI_ASSOC)){
 ...
 echo "</strong>&nbsp;si&nbsp;".$g3."&nbsp;<strong>".$l3."</strong>&nbsp;".$g4."&nbsp;<strong>".$l4."</strong>";echo "&nbsp;procedand primul/primii la predarea si secundul/secunzii la primirea tehnicii si materialelor de comunicatii din cadrul Compartimentului Comunicatii si Informatica.";
 echo "<br />Cu ocazia predarii-primirii au afost aduse la cunostinta si activitatile desfasurate dupa cum urmeaza:<br /><br />";
...
while($rez=mysqli_fetch_array($query1,MYSQLI_ASSOC)){echo "<input type='checkbox' />" ;
echo $rez['lucratori'];
 
 
echo "<br />";}
$url= str_replace("?","&",$_SERVER['REQUEST_URI']);$id=explode('&' , $url );for ($x=0; $x<($cont+1)*2;$x++){if (isset($id[$x])){ 
$z=substr($id[$x],0,2);
if ($z=="a="){
   $d=substr($id[$x],2);
   $querya=mysqli_query($con,"select lucratori from agenti where id='$d'");
   $nume=mysqli_fetch_array($querya,MYSQLI_ASSOC);
   echo $nume['lucratori'];   echo $id[$x]."<br>";
